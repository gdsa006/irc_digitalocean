  ...
  <table class="table">
    ...
    @foreach($leads as $k=>$lead)
      <tr id="lead-{{ $lead->id }}">
        ...
        <td>{{ $lead->address }}</td>
        <td><a class="viewDetails" data-html="true" href="javascript:void(0)" title="Sqft: {{ $lead->sqft }} <br/> Roof: {{ $lead->steep }} <br/> Currently: {{ $lead->existingmaterial }} <br/> Required: {{ $lead->material }} <br/> Urgent: {{ $lead->urgency }} <br/> Insurance: {{ $lead->true }} <br/> Interested in financing: {{ $lead->isinterestedinfinancing }} ">Details</a></td>
